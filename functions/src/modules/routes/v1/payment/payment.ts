...
import { getOrCreateCustomer, getOrderById, getStripePaymentInfo, getTransactionByOrderId, onAccountUpdated, onPaymentFailed, onPaymentSucceeded, updateOrCreateTranasction, updateOrderItemStatus, updateOrderStatus, updateTransactionStatus } from "./helper";
...
router.get("/api/payment/stripe/getCapability", async (req, res) => {
    try {
...
router.post("/api/payment/stripe/connectStripeExpressAccount", async (req, res) => {
    try {
...
router.delete("/api/payment/stripe/deleteConnectAccount", async (req, res) => {
    try {
        const sellerId = req.query.sellerId;
...
router.post("/api/payment/stripe/createPaymentIntent", async(req,res) =>{
    try {
        ...
        const order = (
          ...
          logger.info(`creating payment intent for buyer ${buyer} : customer ${customer} with order ${order}`);
          const paymentIntent = await stripe.paymentIntents.create({
            customer: customer.id,
            payment_method_types: ["card"],
            amount: order.totalInCents,
            currency: currency == undefined || currency == '' ? 'usd' : currency,
            // application_fee_amount: appFeeInCents == undefined  || appFeeInCents == '' ? 100 : appFeeInCents,
